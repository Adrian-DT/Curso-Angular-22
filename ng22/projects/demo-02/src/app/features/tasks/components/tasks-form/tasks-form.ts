...
@Component({
  ...
  template: `
  <form #ngForm="ngForm" (ngSubmit)="ngForm.form.valid && onSubmit(ngForm.value)">
...
export class TasksForm {
  // Evento para añadir una nota a la lista de notas, que se propaga desde el componente hijo (NoteForm) hacia el componente padre (TasksList).
  ...
  onSubmit(formValue: NgForm['value']) {
    // Mostramos en consola los valores del formulario para verificar que se están enviando correctamente.
    console.log('Evento emitido con los valores del formulario:', formValue);
    ...
    // Emitimos el evento con los valores del formulario, que se propaga hacia el componente padre (TasksList) para añadir la nota correspondiente.
    this.addEvent.emit(formValue);
